  ...
  connection.query(selectWordsWithoutMorph, (err, result) => {
    ...
    const tryNextWord = () => {
      ...
      if(result.length > 0) {
        ...
        let checkSingleLemmaStatement = `SELECT DISTINCT lemma FROM words_enhanced WHERE word="${rowWithoutMorph.word}"`
        connection.query(checkSingleLemmaStatement, (err, result) => {
          if(err) throw err

          if(result.length > 1) {
            // unsafe to try to auto-parse a word with multiple lemmas
            console.log(`  Skipping ${rowWithoutMorph.word} since this form is a representation of multiple lemmas and thus unsafe to auto-parse.`)
            tryNextWord()
            return 
          }
